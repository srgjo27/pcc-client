...
import { useEventInformation, useUpdateEvent } from '../hooks';
...
export const EditModal: React.FC<EditModalProps> = ({
    ...
    const { data: event, isLoading: isLoadingEvent } = useEventInformation(id);
    const { mutate, isPending: isUpdating } = useUpdateEvent();
    ...
    useEffect(() => {
        if (isOpen && event) {
            reset({
                ...
                context: event.context as "LECTURE" | "WORK" | "BUSINESS" | "PERSONAL" | "GYM",
                startTime: formatToDatetimeLocal(event.startTime),
                endTime: formatToDatetimeLocal(event.endTime),
                isRecurring: event.isRecurring,
                recurrence: event.isRecurring && event.recurrence
                    ? {
                        frequency: (event.recurrence.frequency || 'WEEKLY').toUpperCase() as "DAILY" | "WEEKLY" | "MONTHLY",
                        days: (event.recurrence.days || []).map((d: string) => d.toUpperCase()) as ("MON" | "TUE" | "WED" | "THU" | "FRI" | "SAT" | "SUN")[],
                    }
                    : null,
                location: event.location || '',
                color: event.color || '#26A69A',
            });
        }
    }, [isOpen, event, reset]);
    ...
    const isPending = isUpdating || isLoadingEvent;

    return (
        <Modal
            isOpen={isOpen}
            onClose={onClose}
            title={lang === 'id' ? 'Ubah Jadwal' : 'Edit Event'}
            size="md"
        >
            {isLoadingEvent ? (
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
                    ...
                    <div className="flex items-center gap-3 py-2 mt-2">
                        ...
                        />
                        <label
                            htmlFor="edit-event-isRecurring"
                    ...
                    <div className="flex flex-col-reverse sm:flex-row justify-end gap-2 pt-4">
                        ...
                        >
                            {!isUpdating && (lang === 'id' ? "Simpan" : "Save")}
                        </Button>
                    </div>
                </form>
            )}
