...
export const ToDo: React.FC = () => {
  ...
  const contexts = [
    { id: 'college', name: t.todo.aside.contexts.college, icon: GraduationCap, count: 3, color: 'bg-indigo-50 text-indigo-600 border-indigo-100 hover:bg-indigo-100/80' },
    { id: 'work', name: t.todo.aside.contexts.work, icon: Briefcase, count: 4, color: 'bg-teal-50 text-teal-600 border-teal-100 hover:bg-teal-100/80' },
    { id: 'business', name: t.todo.aside.contexts.business, icon: TrendingUp, count: 2, color: 'bg-amber-50 text-amber-600 border-amber-100 hover:bg-amber-100/80' },
    { id: 'personal', name: t.todo.aside.contexts.personal, icon: User, count: 1, color: 'bg-rose-50 text-rose-600 border-rose-100 hover:bg-rose-100/80' },
  ];

  const priorities = [
    { id: 'high', name: prioName(t.todo.aside.priorities.high), count: 2, color: 'text-rose-600 bg-rose-50 border-rose-100', dot: 'bg-rose-500' },
    { id: 'medium', name: prioName(t.todo.aside.priorities.medium), count: 5, color: 'text-amber-600 bg-amber-50 border-amber-100', dot: 'bg-amber-500' },
    { id: 'low', name: prioName(t.todo.aside.priorities.low), count: 3, color: 'text-slate-600 bg-slate-50 border-slate-100', dot: 'bg-slate-400' },
  ];

  function prioName(name: string) {
    return name;
  }
          ...
          <Card className="hover:shadow-md transition-shadow duration-300">
            ...
            <CardContent>
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <h3 className="text-sm font-medium">{t.todo.emptyStateTitle}</h3>
                <p className="text-xs text-slate-500 max-w-xs mt-1">
                  {t.todo.emptyStateSubtitle}
                </p>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 pt-2">
                <div className="rounded-lg bg-[#F0F9FF] p-3 flex items-center gap-2.5 border border-neutral-100">
                  ...
                  <div>
                    <p className="text-[10px] font-medium text-slate-500">{t.todo.aside.stats.completed}</p>
                ...
                <div className="rounded-lg bg-amber-50/50 p-3 flex items-center gap-2.5 border border-slate-100">
                  ...
                  <div>
                    <p className="text-[10px] font-medium text-slate-500">{t.todo.aside.stats.pending}</p>
